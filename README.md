...
- **Repeat a block of steps**: select steps and press *Repeat…* — the block is wrapped in ⟳ markers and plays the chosen number of times. Edit the count via *Edit step* on the end marker; delete the two markers to remove the loop; nesting works. (The whole macro is repeated with the *Repeat* property.)
