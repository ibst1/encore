...
- **Copies wait for the clipboard automatically**: Ctrl+C in e.g. Excel completes asynchronously — at playback speed the paste sometimes beat the copy and a row was skipped. A replayed Ctrl+C/Ctrl+X/Ctrl+Insert now snapshots the clipboard sequence number before the keystroke and waits (≤ 1.2 s) after it until the clipboard actually changed. A copy that puts nothing on the clipboard costs the timeout. The explicit *Wait for clipboard* step remains for menu-driven copies and other cases the automatic detection cannot see.
